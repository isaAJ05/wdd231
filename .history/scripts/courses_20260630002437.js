...
// array given by the university with all the courses in the web certificate program
const courses = [
    {
        subject: 'CSE',
        number: 110,
...
function displayCourses(courseList) {
    courseContainer.innerHTML = ""; // clear the container before adding new courses

    courseList.forEach(course => {
        const card = document.createElement("div"); // create a card for each course
        card.classList.add("course-card");

        if (course.completed) {
            card.classList.add("completed");// add a class to indicate the course is completed
        }
...
allBtn.addEventListener("click", () => {
    ...
    const text1 = document.createElement("div"); // create a card for each course
    text1.classList.add("course-card");
});
